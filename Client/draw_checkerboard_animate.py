...
def draw_checkerboard_animate():
    for i in range(connected_panel_height):
        for j in range(connected_panel_width):
            if ((i + j) % 2 == 0):
                draw_transparent_pixel(j,i)
            else:
                draw_light_pixel(j,i)
                
            time.sleep(33/1000)

def clear_screen_animate():
    for i in range(connected_panel_height):
        for j in range(connected_panel_width):
            draw_dark_pixel(j, i)
...
def clear_screen():
    for i in range(connected_panel_width):
        for j in range(connected_panel_height):
            draw_dark_pixel(i,j)
...
for i in range(30):
    ...
    if(i % 2 == 0):
        draw_checkerboard_animate()
    else:
        clear_screen_animate()
